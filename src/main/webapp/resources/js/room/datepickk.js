			...
			[].slice.call(element.querySelectorAll('.d-table input')).forEach(function (inputEl, i) {
				...
				if (date instanceof Date) {
					...
					if (_highlights.length > 0) {
						...
						// 0821 하이라이트 모바일용 변수 추가
						var trans = ''; // transform
						var bgP = ''; // background-position-y
						...
						if (_highlights.length > 1) {
							...
							// 0814 하이라이트 추가 수정
							bg = _highlights[_highlights.length-1].background ? 'background:'+ _highlights[_highlights.length-1].background+';':'';
							trans = _highlights[_highlights.length-1].transform ? 'transform:'+ _highlights[_highlights.length-1].transform+';':'';
							bgP = _highlights[_highlights.length-1].backgroundPositionY ? 'background-position-y:'+ _highlights[_highlights.length-1].backgroundPositionY+';':'';

							// bgColor = _highlights[_highlights.length-1].backgroundColor ? 'background-color:' + _highlights[_highlights.length-1].backgroundColor + ';' : '';
							...
							if (date.getTime() == _highlights[_highlights.length-1].dates[0].start.getTime()) {
								borderRadius = 'border-top-left-radius: 30px; border-bottom-left-radius: 30px;'
							}
							else if (date.getTime() == _highlights[_highlights.length-1].dates[0].end.getTime()) {
								borderRadius = 'border-top-right-radius: 30px; border-bottom-right-radius: 30px;'
							}

						} else {

							// 0814 하이라이트 추가 수정
							bg = _highlights[0].background ? 'background:'+ _highlights[0].background+';' :'';
							trans = _highlights[0].transform ? 'transform:'+ _highlights[0].transform+';' :'';
							bgP = _highlights[0].backgroundPositionY ? 'background-position-y:'+ _highlights[0].backgroundPositionY+';' :'';
						...
						// 0814 라벨 스타일 추가 수정
						labelEl.setAttribute('style', trans + bgP+ bg + bgColor + Color + borderRadius);
				...
				if (x instanceof Array) {
					x.forEach(function (hl) {
						if (hl instanceof Object) {
							...
							// 0814 하이라이트 추가 수정
							highlightObj.background = hl.background;
							highlightObj.backgroundPositionY = hl.backgroundPositionY;
							highlightObj.transform = hl.transform;
					...
					// 0814 하이라이트 추가 수정
					highlightObj.background = x.background;
					highlightObj.backgroundPositionY = x.backgroundPositionY;
					highlightObj.transform = x.transform;
...
var template = '<div class="d-title"></div>' +
			   '<div class="d-calendar">' +
			       '<div class="d-header">' +
			            ...
			            '<i id="d-next"></i>' +
			        '</div>' +
			        '<div class="d-month-picker">' + '<div data-month="1">1</div>' + '<div data-month="2">2</div>' + '<div data-month="3">3</div>' + '<div data-month="4">4</div>' + '<div data-month="5">5</div>' + '<div data-month="6">6</div>' + '<div data-month="7">7</div>' + '<div data-month="8">8</div>' + '<div data-month="9">9</div>' + '<div data-month="10">10</div>' + '<div data-month="11">11</div>' + '<div data-month="12">12</div>' + '</div>' + '<div class="d-year-picker">' + '<div data-year="-5"></div>' + '<div data-year="-4"></div>' + '<div data-year="-3"></div>' + '<div data-year="-2"></div>' + '<div data-year="-1"></div>' + '<div data-year="0"></div>' + '<div data-year="1"></div>' + '<div data-year="2"></div>' + '<div data-year="3"></div>' + '<div data-year="4"></div>' + '<div data-year="5"></div>' + '</div>' + '<div class="d-weekdays"></div>' + '<div class="d-tables"></div>' + '</div>' + '<div class="d-legend"></div>' + '<button class="d-confirm"></button>' + '<div class="d-overlay"></div>';
